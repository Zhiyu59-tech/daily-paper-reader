...
tldr: "针对大语言模型推理时计算扩展中离散搜索效率低下的问题，本文提出 ∇-Reasoner 框架。该方法引入可微文本优化（DTO），在解码循环中利用模型似然和奖励模型的梯度信号，在潜在空间内通过梯度下降实时优化生成策略。实验表明，该方法在数学推理任务上准确率提升超 20%，且显著降低了计算开销，实现了从零阶搜索到一阶优化的范式转变。"
selection_source: fresh_fetch
motivation: 现有的推理时扩展方法主要依赖低效的离散搜索或试错，难以充分利用连续空间的优化潜力。
method: 提出一种迭代生成框架，通过在 token 逻辑值上进行可微优化，结合梯度下降、拒绝采样和加速设计来精炼文本表示。
result: "在挑战性数学推理基准上实现了 20% 以上的准确率提升，同时减少了 10-40% 的模型调用次数。"
conclusion: 该工作证明了在推理阶段采用一阶梯度优化比传统搜索更高效，为增强 LLM 推理能力开辟了新路径。
---
