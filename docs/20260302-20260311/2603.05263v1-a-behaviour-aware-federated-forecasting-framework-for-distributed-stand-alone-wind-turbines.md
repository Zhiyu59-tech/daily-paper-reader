...
motivation: 传统的集中式风电预测面临数据隐私泄露、传输成本高以及风机异构性带来的挑战。
method: 提出两阶段联邦学习框架，先通过DRS和递归自动拆分进行行为聚类，再利用FedAvg训练集群专属的LSTM模型。
result: 在400台风机的实验中，该框架成功识别了行为一致的群体，预测精度优于地理分区方法并达到基准水平。
conclusion: 行为感知的联邦学习是解决异构分布式风机预测中隐私与性能平衡问题的有效途径。
---
